...
export const useQueryRoleDetails = () => {
  const { roleId } = useAtomValueRoleDetails();

  const { data, isFetching, refetch } = useQuery(queryKey, () => SysRoleGetInfo({ roleId }), {
    enabled: false,
  });
...
export const useEditRoleDetails = () => {
